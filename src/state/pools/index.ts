...
export const fetchPoolsPublicDataAsync = (currentBlock: number) => async (dispatch, getState) => {
  ...
  const liveData = poolsConfig.map((pool) => {
    ...
    const stakingTokenPrice = 0.0000838

    const earningTokenAddress = pool.earningToken.address ? pool.earningToken.address.toLowerCase() : null
    let earningTokenPrice = 0.000

    if (
      earningTokenAddress !== undefined &&
      earningTokenAddress.toLowerCase() === '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56'.toLowerCase()
    ) {
      earningTokenPrice = 1
    }
  ...
  reducers: {
    setPoolsPublicData: (state, action) => {
      ...
      state.data = state.data.map((pool) => {
       ...
       // console.log({ ...pool, ...livePoolData },"pool info")
